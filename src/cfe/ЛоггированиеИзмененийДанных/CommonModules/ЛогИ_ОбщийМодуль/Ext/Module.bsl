...
Функция ЭтоИтерируемыйТип(Значение) Экспорт
    Перем ТипЗначения;
...
Процедура ЛогИ_ОбъектПередЗаписью(Источник, Отказ) Экспорт
    ...
    Тогда
        Попытка
            ...
            ИначеЕсли
                СтрНачинаетсяС(ТипИсточника, "План видов расчета объект")
            Тогда
                Реквизиты                           =   ЗначенияСвойствСправочника(Источник);
            КонецЕсли;
            Если
                ЭтоИтерируемыйТип(Реквизиты)
            Тогда
...
Процедура ЛогИ_ПланВидовРасчетаМенеджерПередЗаписью(Источник, Отказ) Экспорт
    ЛогИ_ОбъектПередЗаписью(Источник, Отказ);
КонецПроцедуры

// Открывает журнал регистрации с отбором по текущему документу/объекту
Процедура ОткрытьЖурналРегистрацииПоОбъекту(ТекущийОбъект) Экспорт
    
    Если ТекущийОбъект = Неопределено Тогда
        Предупреждение("Не удалось определить текущий объект для отбора в журнале регистрации.");
        Возврат;
    КонецЕсли;
    
    // Создаем отбор для журнала регистрации
    Отбор = Новый Структура;
    
    // Добавляем отбор по событию "Лог. Изменение данных"
    Отбор.Вставить("Событие", "Лог. Изменение данных");
    
    // Добавляем отбор по данным (текущий объект)
    Отбор.Вставить("Данные", ТекущийОбъект);
    
    // Открываем журнал регистрации с отбором
    ОткрытьФорму("Обработка.ЖурналРегистрации.Форма", Новый Структура("Отбор", Отбор));
    
КонецПроцедуры

// Команда для вызова из форм объектов
Процедура КомандаОткрытьЖурналРегистрации(Форма) Экспорт
    
    ТекущийОбъект = Неопределено;
    
    // Пытаемся получить объект из формы
    Если Форма.Свойство("Объект") Тогда
        ТекущийОбъект = Форма.Объект.Ссылка;
        
        // Если ссылка пустая, используем сам объект
        Если Не ЗначениеЗаполнено(ТекущийОбъект) Тогда
            ТекущийОбъект = Форма.Объект;
        КонецЕсли;
        
    ИначеЕсли Форма.Свойство("Запись") Тогда
        // Для форм записей регистров
        ТекущийОбъект = Форма.Запись;
        
    ИначеЕсли Форма.Свойство("Константа") Тогда
        // Для форм констант
        ТекущийОбъект = Форма.Константа;
        
    КонецЕсли;
    
    ОткрытьЖурналРегистрацииПоОбъекту(ТекущийОбъект);
